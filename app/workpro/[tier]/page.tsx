...
export default function WorkProTierPage({ params }: { params: Promise<{ tier: string }> }) {
  ...
  useEffect(() => {
    const fetchTier = async () => {
      // Fetch ONLY WorkPro items of this tier
      ...
      <div className="pt-32 pb-12 px-6 border-b border-white/5 bg-[#1A1A1A]">
        <div className="max-w-7xl mx-auto text-center">
          <h1 className="font-orbitron font-bold text-4xl text-white mb-2">
            WORKPRO <span className="text-brand-purple">LEVEL {tier}</span>
          </h1>
          <p className="text-brand-silver">Professional Workstations.</p>
        </div>
      </div>

      <div className="flex-grow max-w-7xl mx-auto px-6 py-12 w-full">
        {loading ? (
            <div className="text-center text-brand-silver">Loading workstations...</div>
        ) : products.length === 0 ? (
            <div className="text-center text-brand-silver border border-dashed border-white/10 py-12 rounded">No workstations found in this tier yet. Check back soon.</div>
        ) : (
