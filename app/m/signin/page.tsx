  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        ...
        // EXPLICIT SANDBOX ROUTE: Forces the mobile dashboard
        router.push("/m"); 
      }
    } catch (err: any) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        ...
        options: { redirectTo: `${window.location.origin}/m/dashboard` },
      });
      if (error) throw error;
    } catch (err: any) {
        ...
        <div className="flex-grow overflow-y-auto overflow-x-hidden relative bg-[#121212] [&::-webkit-scrollbar]:w-[6px] [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-[#B084FF] [&::-webkit-scrollbar-thumb]:rounded-l-full pr-0">
          ...
          <MotionWrapper className="px-6 py-8 flex flex-col min-h-full">
            ...
            <div className="flex justify-between items-center mb-10">
              ...
              {/* FIX 3: Standardized links for seamless middleware operation */}
            ...
            <form onSubmit={handleLogin} className="flex flex-col flex-grow">
              <div className="space-y-6 mb-4">
                <div>
                  ...
                  <input 
                    ...
                    // FIX 4: Removed invalid placeholder class
              ...
              <div className="flex justify-end mb-8">
                <Link href="/forgot-password" className="text-white text-xs font-saira hover:text-[#B084FF] transition-colors tracking-wide">Forgot Password??</Link>
              </div>

              <button 
                type="submit"
                disabled={loading}
                className="w-full border border-white/20 hover:border-[#B084FF] disabled:opacity-50 disabled:active:scale-100 bg-[#121212] py-4 rounded-xl text-white font-saira text-sm tracking-widest uppercase transition-all mb-10 shadow-[0_0_15px_rgba(78,44,139,0.15)] active:scale-[0.98]"
              >
            ...
            <div className="mt-auto text-center pb-4">
              <p className="text-white text-sm font-saira">
                New to Rig Builders? <Link href="/signup" className="text-[#B084FF] hover:underline underline-offset-4 decoration-white/30 ml-1">Register Now</Link>
              </p>
            </div>
